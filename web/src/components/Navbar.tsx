...
import { GraduationCap, LayoutDashboard, Target, MessageSquare, BookOpen, Users, User, Settings, History, LogOut, ChevronDown, Newspaper } from "lucide-react";
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 shadow-sm">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 relative" ref={dropdownRef}>
          ...
          >
            <div className="w-8 h-8 rounded-full bg-blue-100 border border-blue-200 flex items-center justify-center shrink-0 text-[#2563EB]">
              <User className="w-4 h-4" />
            </div>
            <span className="text-sm font-bold text-slate-700 hidden sm:block">Nguyễn Văn A</span>
            ...
            {isDropdownOpen && (
              ...
              >
                <div className="px-4 py-3 border-b border-slate-100 mb-2 bg-[#2563EB]/5">
                  <p className="text-sm font-bold text-slate-900 truncate">Nguyễn Văn A</p>
                  <p className="text-xs text-blue-700 font-medium truncate">Sinh viên HUIT</p>
                </div>
                ...
                <div className="border-t border-slate-100 mt-2 pt-2">
                  <Link href="/auth/login" onClick={() => setIsDropdownOpen(false)} className="flex items-center gap-3 px-4 py-2 text-sm font-bold text-red-600 hover:bg-red-50 transition">
