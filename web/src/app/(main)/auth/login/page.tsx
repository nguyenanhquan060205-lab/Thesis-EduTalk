...
function LoginFormContent() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (apiDetail) {
        setError(`Lỗi xác thực máy chủ: ${apiDetail}`);
      } else if (errCode === "auth/unauthorized-domain") {
        setError("Firebase chỉ cho phép đăng nhập Google tại địa chỉ http://localhost:3000 (tránh dùng IP mạng 192.168.x.x).");
      } else if (errCode === "auth/popup-closed-by-user") {
        setError("Bạn đã đóng cửa sổ đăng nhập Google.");
      } else if (errCode === "auth/popup-blocked") {
        setError("Trình duyệt chặn mở popup. Vui lòng bấm vào thanh địa chỉ để cho phép mở popup.");
      } else {
        setError((err as { message?: string })?.message || "Không thể kết nối đến tài khoản Google.");
      }
    } finally {
      setLoading(false);
    }
        ...
        <form className="space-y-4" onSubmit={handleEmailLogin}>
          ...
          <div className="pt-2">
            <motion.button 
              ...
              type="submit" 
              disabled={loading}
              className="w-full py-3.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs flex items-center justify-center gap-2 transition shadow-md shadow-blue-500/20 disabled:opacity-50 cursor-pointer"
            >
          ...
          <motion.button 
            ...
            disabled={loading}
            className="w-full py-3 px-4 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-700 font-extrabold text-xs flex items-center justify-center gap-3 transition shadow-xs disabled:opacity-50 cursor-pointer"
          >
