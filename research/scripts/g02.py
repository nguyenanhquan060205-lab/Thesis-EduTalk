...
Lọc hồ sơ trúng tuyển và trải điểm thi ra 10 cột cùng lược đồ với khảo sát.
...
''' + so_do(
    "02_ChuanBiTTTH",
    ["data/raw/TTTH_THPT_raw.xlsx     —  18.024 hồ sơ",
...
thứ mà 676 phiếu khảo sát không thể có nhiều. Đây là phần thông tin thật mà GA sẽ dựa vào.

Nhưng nó **không có** 10 câu Likert, không có giới tính, không có mục tiêu nghề nghiệp. Ba nhóm cột
đó sẽ do GA sinh ra ở Giai đoạn 4, học từ chính phiếu khảo sát của cùng ngành.
...
tho = pd.read_excel(RAW / "TTTH_THPT_raw.xlsx")
...
luu(fig, OUT, "hinh_2_2_do_phu.png",
    ...
    + ". Những ngành này ở Giai đoạn 4 sẽ phải sinh dữ liệu từ chính phiếu khảo sát của chúng.")
plt.show()'''))
...
tom_tat("GIAI ĐOẠN 2 — CHUẨN BỊ HỒ SƠ TRÚNG TUYỂN", [
    ...
    "   → Giai đoạn 4 sẽ dùng GA sinh 3 nhóm cột đó",
])
